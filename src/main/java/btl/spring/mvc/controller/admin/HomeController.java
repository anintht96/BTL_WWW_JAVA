...
public class HomeController {
	...
	public ModelAndView qlDonHang() {
		List<DonHang> list=donHangDAO.listDonHangs();
		return new ModelAndView("admin/donHang","ds",list);
	}
	
	@RequestMapping(value = "/admin-chiTietDH",method = RequestMethod.POST)
	public ModelAndView donHang(@RequestParam("idDonHang") int id,Model model) {
		List<DonHang> list=donHangDAO.listDonHangs();
		model.addAttribute("ds", list);
		List<CTDonHang> listCtDonHangs=ctDonHangDAO.listCtDonHangs(id);
		return new ModelAndView("admin/donHang", "dsDH", listCtDonHangs);
	}
}
